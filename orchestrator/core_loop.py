...
async def run_chat_tools(objective: str, project_id: int | None, run_id: str, max_tool_calls: int = 10) -> dict:
    ...
    # Use environment variable to control model selection, default to GPT-4 for reliability
    ...
    logger.info("Created ChatOpenAI instance with model: %s", model_name)
    ...
    for tool in LC_TOOLS:
        if hasattr(tool, 'name') and hasattr(tool, 'description') and hasattr(tool, 'args_schema'):
            ...
            logger.info("Valid tool: %s - %s (schema: %s)", tool.name, tool.description, tool.args_schema.__name__)
        else:
    ...
    logger.info("Successfully bound %d tools to LLM", len(valid_tools))
...
"""
    ...
    user_message = objective if project_id is None else f"{objective}\n\nproject_id={project_id}"
    ...
    for iteration in range(max_tool_calls):
        ...
        if tcs:
            ...
            for i, tc in enumerate(tcs):
                ...
                logger.info("Processing tool call %d: %s with args %s (id: %s)", i + 1, name, args, call_id)
                ...
                tool = next((t for t in valid_tools if getattr(t, "name", None) == name), None)
                if tool is None:
                ...
                try:
                    ...
                    logger.error("Tool '%s' execution failed: %s", name, str(e), exc_info=True)
        ...
        if response_content.lower() in ['placeholder', 'i\'ll help you', 'i can help']:
            logger.warning("LLM returned placeholder response instead of using tools: %s", response_content)
